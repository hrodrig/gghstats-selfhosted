...
- Default **gghstats** container image tag **`v1.0.0`** ([gghstats v1.0.0](https://github.com/hrodrig/gghstats/releases/tag/v1.0.0) — stable default SQLite path, SPEC freeze, alert dash URL fix, access-log client IP): Compose, Helm, `.env.example`, Linux standalone `.deb`/`.rpm` examples, platform-test defaults.
- **Helm chart:** bump **`version:`** to **0.1.38**, **`appVersion`** to **1.0.0**.
- **README:** sync version badge to **0.1.52**.

## [0.1.51] - 2026-07-23

### Changed

- Default **gghstats** container image tag **`v0.11.0`** ([gghstats v0.11.0](https://github.com/hrodrig/gghstats/releases/tag/v0.11.0) — API-only mode, dogfood JSON, CORS, CSP Report-Only, SYNC+ date-scoped deltas): Compose, Helm, `.env.example`, Linux standalone `.deb`/`.rpm` examples, platform-test defaults.
- **Helm chart:** bump **`version:`** to **0.1.37**, **`appVersion`** to **0.11.0**.
- **README:** sync version badge to **0.1.51**.
