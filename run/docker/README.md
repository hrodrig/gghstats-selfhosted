...
The image tag must match a published **GHCR** image from [gghstats releases](https://github.com/hrodrig/gghstats/releases) — same value as **`GGHSTATS_VERSION`** in **[`run/common/.env.example`](../common/.env.example)** (e.g. `v0.1.5`). For **all** environment variables supported by the binary/container, see the upstream **[`.env.example`](https://github.com/hrodrig/gghstats/blob/main/.env.example)** on **`main`** in the gghstats repository.
