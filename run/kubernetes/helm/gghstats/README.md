...
**Validation:** **`values.schema.json`** requires **`resources.requests`** and **`resources.limits`** (cpu + memory). `helm install` / `helm template` fail if `resources` is omitted or cleared (e.g. `--set resources=null`). On GitHub, [helm-lint.yml](https://github.com/hrodrig/gghstats-selfhosted/blob/main/.github/workflows/helm-lint.yml) runs **`helm lint`** and **`kubectl apply --dry-run=client`** on rendered manifests for several scenarios whenever **`run/kubernetes/helm/`** changes.
