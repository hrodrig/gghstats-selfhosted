...
| *(Optional)* **Custom dashboard CSS** | [Repository README — Custom UI theme (optional)](../README.md#custom-ui-theme-optional) — **`GGHSTATS_CUSTOM_CSS`**, image **≥ 0.2.0** (defaults use **`v1.6.1`**). |
| *(Optional)* **Repo pins + Featured showcase** | Upstream [gghstats Catalog & Featured](https://github.com/hrodrig/gghstats/blob/main/docs/catalog-and-featured.md) — `gghstats repo add` / `featured add` curate the traffic set and a `/featured` page (image **≥ 1.1.0**). |
